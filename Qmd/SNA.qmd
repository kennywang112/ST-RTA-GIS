...
get_neighbor_features <- function(labeled_features, all_features_grid) {
  
  extract_neighbors <- function(target_category, label_name) {

    target_nodes <- labeled_features %>%
      filter(final_category == target_category)

    if (nrow(target_nodes) == 0) {
      warning(paste("沒有類別為", target_category, "的節點"))
      return(NULL)
    }
    neighbor_indices <- unique(unlist(target_nodes$neighbors))
    
    if (length(neighbor_indices) == 0) {
      warning(paste("類別", target_category, "沒有鄰居"))
      return(NULL)
    }
  
    neighbor_data <- all_features_grid[neighbor_indices, ] %>%
      mutate(
        parent_group = label_name,
        role = "Neighbor"
      )
  ...
  green_neighbors <- extract_neighbors(
    "Hot_with_Safe_Neighbors_highbetweenness", 
    "Broken_Neighbors"
  )

  orange_neighbors <- extract_neighbors(
    "Cold_with_Hot_Neighbors_highbetweenness", 
    "Resilient_Neighbors"
  )
  
  comparison_neighbors <- bind_rows(green_neighbors, orange_neighbors)
  
  return(comparison_neighbors)
}

neighbor_result <- get_neighbor_features(df, final_all_features)
```
