...
![](data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABIAAAASCAYAAABWzo5XAAAAbElEQVR4Xs2RQQrAMAgEfZgf7W9LAguybljJpR3wEse5JOL3ZObDb4x1loDhHbBOFU6i2Ddnw2KNiXcdAXygJlwE8OFVBHDgKrLgSInN4WMe9iXiqIVsTMjH7z/GhNTEibOxQswcYIWYOR/zAjBJfiXh3jZ6AAAAAElFTkSuQmCC){alt=""}
...
## 4. This is to classify using grid-based data (all_features.csv)
...
# This is for original version, now we're testing v2
# cols_to_remove <- c(
...
# This is for new version, now we're testing v1
...
```{r}
# This is new: to transform proportion to category
...
bad_group_balanced <- bad_group_balanced %>%
  mutate(across(
...
res_group_balanced <- res_group_balanced %>%
  mutate(across(
    .cols = where(is.numeric) & !any_of(exclude_cols),
...
tree_bad_model <- rpart(Group_Label ~ . , 
                    ...
                    method = "class",
                    maxdepth = 4,
                    control = rpart.control(cp = 0.02, minbucket = 30),
...
png("../Layouts/tree_bad_model_cat.png", width = 1200, height = 800)
...
png("../Layouts/tree_res_model_cat.png", width = 1200, height = 900)
