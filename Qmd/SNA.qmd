...
get_four_categories_data <- function(labeled_features, all_features_grid) {
  ...
  process_group <- function(self_category_name, neighbor_category_name) {
  
    # 先找出目標節點
    self_nodes <- labeled_features %>% filter(final_category == self_category_name)
    
    if (nrow(self_nodes) == 0) return(NULL)
    ...
    valid_neighbor_node_indices <- unique(unlist(lapply(self_nodes$neighbors, function(nbs) {
      ...
      setdiff(nbs, self_node_indices)
    })))
    
    if (length(valid_neighbor_node_indices) == 0) {
      warning(paste(self_category_name, "沒有符合標準的鄰居"))
      return(NULL)
    }
...
pred_class <- predict(tree_model, newdata = bad_group, type = "class")
conf_matrix <- confusionMatrix(data = pred_class, reference = as.factor(bad_group$Group_Label))
...
pred_class <- predict(tree_model, newdata = res_group, type = "class")
conf_matrix <- confusionMatrix(data = pred_class, reference = as.factor(res_group$Group_Label))
