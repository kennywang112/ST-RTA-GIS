...
all_features_mean <- final_data_for_tree %>%
  filter(Group_Label == 'Broken_Overlap' | Group_Label == 'Resilient_Overlap')%>%
  group_by(Group_Label) %>%
...
l <- c('bn_feature', 'lr_feature', 'hotspot')
diff_analysis <- comparison_table %>%
  mutate(Diff_Broken = abs(Broken_Overlap - Resilient_Overlap)) %>% 
  arrange(desc(Diff_Broken))%>%
  head(20)
...
diff_analysis %>%
  ...
  ggplot(aes(x = Feature, y = Value, fill = Group)) +
